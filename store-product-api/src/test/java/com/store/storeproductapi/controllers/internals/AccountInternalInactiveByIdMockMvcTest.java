...
class AccountInternalInactiveByIdMockMvcTest {
    ...
    void internalAccountsAccountIdInactiveDelete() throws Exception {
        ...
        final String accountId = accountTO.getId();

        when(this.accountControllerHelper.internalAccountsAccountIdInactiveDelete(accountId))
                .thenReturn(accountTO);

        this.mockMvc.perform(delete(ApiTestConstants.INTERNAL_ACCOUNTS_WITH_ID_INACTIVE, accountId))
                .andExpect(status().isAccepted())
                .andExpect(jsonPath("subjectId").value(accountTO.getSubjectId()))
                .andExpect(jsonPath("username").value(accountTO.getUsername()));

        verify(this.accountControllerHelper).internalAccountsAccountIdInactiveDelete(accountId);
    }
